...
namespace Gboxt.Common.WebUI
{
        ...
        where TBusinessLogic : UiBusinessLogicBase<TData, TAccess>, new()
    {
        ...
        ///     取得列表数据
        /// </summary>
        protected void LoadListData(string condition, DbParameter[] args)
        {
            ...
            ModelContext.IsUnSafeMode = true;
            var data = Business.PageData(page, rows, sort, desc, condition, args);
            var ed = new EasyUiGridData<TData>
            {
                Succeed = true,
                Data = data.ResultData.Rows,
                Total = data.ResultData.RowCount
            };
            AjaxResult = ed;
            if (OnListLoaded(data.ResultData.Rows, data.ResultData.RowCount))
            {
                CheckListResult(ed, condition, args);
            }
        }
        ...
        private void SaveQueryArguments(int page, string sort, string adesc, int rows)
        {
            ...
            ModelContext.PowerChecker.SaveQueryHistory(LoginUser, PageItem, requestArgs);
        }
        ...
        protected virtual bool CheckListResult(EasyUiGridData<TData> result, string condition, params DbParameter[] args)
        {
            return true;
        }
        ...
        protected virtual void AddNew()
        {
            ...
            {
                SetFailed(/*"数据不正确,保存失败<br/>" +*/ ModelContext.GetFullMessage());
                return;
            }
        ...
        private void DoDelete()
        {
            ...
            SetFailed(ModelContext.GetFullMessage());
        }
