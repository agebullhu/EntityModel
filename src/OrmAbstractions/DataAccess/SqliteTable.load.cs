...
namespace Agebull.Orm.Abstractions
{
    partial class DataTable<TData, TSqliteDataBase>
    {
        ...
        protected object CollectInner(string fun, string field, string condition, params DbParameter[] args)
        {
            ...
            {
                return OrmDataBase.ExecuteScalar(sql, args);
            }
        }
            ...
            DbParameter[] args)
        {
            ...
            {
                using (var cmd = OrmDataBase.CreateCommand(sql, args))
                {
        ...
        {
            return Page(page, limit, order, desc, condition, args.Cast<DbParameter>().ToArray());
        }
            ...
            Expression<Func<TData, bool>> lambda)
        {
            ...
            {
                using (var cmd = OrmDataBase.CreateCommand(sql, convert.Parameters))
                {
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
        ...
        public List<TData> LoadData(MulitCondition condition)
        {
            ...
            {
                var pa = new DbParameter(item.Name, item.Type);
                if (item.Value == null)
        ...
        private TData LoadEntity(ReaderScope reader)
        {
        ...
        public List<TData> LoadDataByProcedure(string procedure, DbParameter[] args)
        {
            ...
            {
                using (var cmd = OrmDataBase.CreateCommand(procedure, args))
                {
