...
{
    partial class DataTable<TData, TSqliteDataBase>
    {
        #region 实体更新
        ...
        protected bool InsertInner(TData entity)
        {
            ...
            using (var cmd = OrmDataBase.CreateCommand())
            {
                var isIdentitySql = SetInsertCommand(entity, cmd);
                OrmDataBase.TraceSql(cmd);
                if (isIdentitySql)
                {
                ...
                if (!string.IsNullOrEmpty(sql))
                    using (var cmd2 = OrmDataBase.CreateCommand())
                    {
...
{AfterUpdateSql(PrimaryKeyConditionSQL)}";
        ...
        public bool PhysicalDelete(object key)
        {
            ...
            using (var scope = TransactionScope.CreateScope(DataBase))
            {
                ...
                var result = OrmDataBase.Execute($@"DELETE FROM [{WriteTableName}] WHERE {condition};", para);
                if (result == 0)
                    return false;
        ...
        public int PhysicalDelete(Expression<Func<TData, bool>> lambda)
        {
            ...
            using (var scope = TransactionScope.CreateScope(DataBase))
            {
                ...
                cnt = OrmDataBase.Execute(CreateDeleteSql(convert), convert.Parameters);
                if (cnt == 0)
                    return 0;
        ...
        private int DeleteInner(string condition, params DbParameter[] args)
        {
            ...
            if (!string.IsNullOrEmpty(condition))
                return OrmDataBase.Execute(CreateDeleteSql(condition), args);
            throw new ArgumentException(@"删除条件不能为空,因为不允许执行全表删除", GetType().FullName);
        }
        ...
        public int SetCoustomValue<TKey>(string valueExpression, TKey key)
        {
            ...
            using (var scope = TransactionScope.CreateScope(DataBase))
            {
                ...
                result = OrmDataBase.Execute(sql, arg2);
                if (result == 0)
                    return 0;
        ...
        private int DoUpdateValue(string field, object value, string condition, DbParameter[] args)
        {
            ...
            using (var scope = TransactionScope.CreateScope(DataBase))
            {
                ...
                result = OrmDataBase.Execute(sql, arg2.ToArray());
                if (result <= 0)
                    return 0;
        ...
        public int SetMulitValue(string expression, string condition, DbParameter[] args)
        {
            ...
            using (var scope = TransactionScope.CreateScope(DataBase))
            {
                ...
                result = OrmDataBase.Execute(sql, args);
                if (result == 0)
                    return 0;
                OnOperatorExecutd(condition, args, DataOperatorType.MulitUpdate);
                scope.SetState(true);
        ...
        /// <param name="args">参数值</param>
        /// <param name="operatorType">操作类型</param>
        protected virtual void OnOperatorExecutd(DataOperatorType operatorType, string condition, IEnumerable<DbParameter> args)
        {
        }
        ...
        /// <param name="args">参数值</param>
        /// <param name="operatorType">操作类型</param>
        private void OnOperatorExecutd(string condition, IEnumerable<DbParameter> args, DataOperatorType operatorType)
        {
        ...
        {
            if (GlobalEvent)
                DataUpdateHandler.EventProxy.OnStatusChanged(OrmDataBase.Name, Name, operatorType, EntityEventValueType.Key, key?.ToString());
        }
        ...
        private void OnMulitUpdateEvent(DataOperatorType operatorType, string condition, DbParameter[] args)
        {
            ...
            DataUpdateHandler.EventProxy.OnStatusChanged(OrmDataBase.Name, Name, operatorType, EntityEventValueType.QueryCondition, JsonConvert.SerializeObject(queryCondition));
        }
        /// <summary>
        ///     更新语句后处理(单个实体操作不引发)
        /// </summary>
        ...
        {
            if (GlobalEvent)
                DataUpdateHandler.EventProxy.OnStatusChanged(OrmDataBase.Name, Name, operatorType, EntityEventValueType.EntityJson, JsonConvert.SerializeObject(entity));
        }
