...
namespace Agebull.EntityModel.MySql
{
    ...
    internal class DataStateTrigger : IDataTrigger
    {
        ...
        void IDataUpdateTrigger.AfterUpdateSql<TEntity>(DataAccessOption<TEntity> option, string condition, StringBuilder code)
        {
            bool hase = false;
            ...
            {
                innerCode.Append($@"
    `{option.FieldMap[nameof(IHistoryData.LastReviserId)]}` = {GlobalContext.Current.User.UserId},
    `{option.FieldMap[nameof(IHistoryData.LastModifyDate)]}` = Now()");
    ...
    //        }
            ...
            {
                code.AppendLine($@"
UPDATE `{ option.WriteTableName}` SET
{innerCode}");
...
WHERE {condition}");
        ...
        /// </summary>
        /// <param name="option">当前数据操作对象</param>
        /// <param name="conditions">附加的条件集合</param>
        /// <returns></returns>
        void IDataUpdateTrigger.ConditionSqlCode<TEntity>(DataAccessOption<TEntity> option, List<string> conditions)
        {
            //是否启用数据范围限制
            ...
            {
                code.Append($"`{option.FieldMap[nameof(IAuthorData.AuthorId)]}` = '{GlobalContext.Current.User.UserId}'");
                first = false;
            }
            //部门数据边界过滤
                ...
                GlobalContext.Current.User is IDepartmentData departmentData && !string.IsNullOrEmpty(departmentData.DepartmentId))
            {
                if (scopeData.DataScope.HasFlag(DataScopeType.Lower))
                {
                    ...
                    {
                        code.Append($@"(`{option.FieldMap[nameof(IDepartmentData.DepartmentId)]}` <> '{departmentData.DepartmentId}' AND 
`{option.FieldMap[nameof(IDepartmentData.DepartmentCode)]}` LIKE '{departmentData.DepartmentCode}%')");
                    }
                    else
                    {
                        code.Append($"`{option.FieldMap[nameof(IDepartmentData.DepartmentCode)]}` LIKE '{departmentData.DepartmentCode}%'");
                    }
                }
                else if (scopeData.DataScope.HasFlag(DataScopeType.Home))
                {
                    if (first)
                        ...
                        code.Append(" OR ");
                    code.Append($"`{option.FieldMap[nameof(IDepartmentData.DepartmentId)]}` = '{departmentData.DepartmentId}'");
                }
            }
            //组织数据边界过滤
                ...
                GlobalContext.Current.User is IOrganizationData organizationData && !string.IsNullOrEmpty(organizationData.OrganizationId))
            {
                if (first)
                {
                    conditions.Add($"`{option.FieldMap[nameof(IOrganizationData.OrganizationId)]}` = '{organizationData.OrganizationId}'");
                }
                else
                {
                    conditions.Add($"`{option.FieldMap[nameof(IOrganizationData.OrganizationId)]}` = '{organizationData.OrganizationId}' AND ({code})");
                }
            }
        }
    }
