...
namespace ConsoleTest
{
    class Program
    {
        ...
        private static void Unit(string url, string bear = "Bear *A5858AB8376D4D8D8099D0F49CBE810E_ANDROID_APP")
        {
                ...
                try
                {
                    ...
                    using (var res = req.GetResponse())
                    {
                        using (var stream = res.GetResponseStream())
                        {
                            ...
                            {
                                bytes = new byte[8192];
                                var len = stream?.Read(bytes, 0,(int)8192);
                                while(len ==8192)
                                    len = stream?.Read(bytes, 0, (int)8192);
                                stream?.Close();
