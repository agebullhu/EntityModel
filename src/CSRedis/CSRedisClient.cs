...
namespace CSRedis {
	public partial class CSRedisClient {
	    ...
	    {
	        {
	            ClusterRule =key => ClusterKeys.First();
	    ...
	    internal T ExecuteScalar<T>(Func<RedisConnection2, T> hander)
	    {
	        var pool = ClusterNodes.First().Value;
	        return GetConnectionAndExecute(pool, conn => hander(conn));
	    }
		...
		public long Increment(string key, long value = 1) => 
		    ExecuteScalar(key, (c, k) => c.Client.IncrBy(k, value));
