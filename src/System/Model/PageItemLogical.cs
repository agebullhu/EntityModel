...
{
    public class PageItemLogical : UiBusinessLogicBase<PageItemData, PageItemDataAccess>
    {
        #region 动作保存
        ...
        protected override bool OnSaving(PageItemData data, bool isAdd)
        {
            ...
            if (data.ParentId < 0 || data.ItemType == PageItemType.Root)
                data.ParentId = 0;
            return true;
        }
        ...
        public void FindAndBindType(long pid, List<Type> types)
        {
            ...
            var children = Access.LoadValues(p => p.Id, p => p.ParentId == pid && p.ItemType <= PageItemType.Page);
        ...
        {
            foreach (int id in Access.LoadValues(p => p.Id, lambda))
