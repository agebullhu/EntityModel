...
namespace Agebull.EntityModel.MySql
{
    ...
    public class MySqlSqlBuilder<TData, TDataBase> : ISqlBuilder, IParameterCreater
    {

        public string Condition(string fieldName, string paraName) => $"(`{SqlOption.FieldMap[fieldName]}` = ?{paraName})";

        public string Condition(string fieldName, string paraName, string condition) => $"(`{SqlOption.FieldMap[fieldName]}` {condition} ?{paraName})";

        /// <summary>
        /// Sql对应的配置信息
        /// </summary>
        public DataAccessSqlOption SqlOption { get; set; }
        ...
        public string PrimaryKeyConditionSQL => throw new NotImplementedException();
...
{AfterUpdateSql(condition)}";
        ...
        /// <param name="condition"></param>
        /// <returns></returns>
        public string ConditionSqlCode(string condition)
        {
            if (DataAccess.NoInjection)
            {
            ...
            if (!DataAccess.NoInjection)
            {
                ...
                //DataUpdateHandler.ConditionSqlCode(Table, conditions);
            }
        ...
        public string CreateCollectSql(string fun, string field, string condition)
        {
            if (field != "*")
                field = $"`{SqlOption.FieldMap[field]}`";
            var sql = $@"SELECT {fun}({field}) FROM {SqlOption.ReadTableName}{ConditionSqlCode(condition)};";
            return sql;
        }
        ...
        public string CreateLoadValueSql(string field, string condition)
        {
            ...
            return $@"SELECT `{SqlOption.FieldMap[field]}` FROM {SqlOption.ReadTableName}{ConditionSqlCode(condition)};";
        }
...
FROM {SqlOption.ReadTableName}{ConditionSqlCode(convert.ConditionSql)};";
        }
...
FROM {SqlOption.ReadTableName}{ConditionSqlCode(condition)}
...
{AfterUpdateSql(condition)}";
        ...
        public string OrderSql(bool desc, string field)
        {
            return $"`{SqlOption.FieldMap[field]}` {(desc ? "DESC" : "")}";
        }
        ...
        public DbParameter CreateDbParameter(string name, string typeFile, object value)
            => new MySqlParameter
            {
                ParameterName = name,
        ...
        public ConditionItem Compile<TData1>(Dictionary<string, string> map, Expression<Func<TData1, bool>> lambda)
        {
            throw new NotImplementedException();
        }

        public ConditionItem Compile<TData1>(Dictionary<string, string> map, LambdaItem<TData1> lambda)
        {
            throw new NotImplementedException();
        }
        #endregion
