...
namespace Agebull.EntityModel.Events
{
    ...
    public static class DataUpdateHandler
    {
        ...
        /// <param name="operatorType">操作类型</param>
        public static void OnOperatorExecuting<TEntity>(DataAccessOption<TEntity> option, string condition, IEnumerable<DbParameter> args, DataOperatorType operatorType)
                      where TEntity : class,IDataObject, new()
        {
            foreach (var trigger in DependencyHelper.GetServices<IDataTrigger>())
        ...
        /// <param name="args">参数值</param>
        /// <param name="operatorType">操作类型</param>
        public static void OnOperatorExecuted<TEntity>(DataAccessOption<TEntity> option, string condition, IEnumerable<DbParameter> args, DataOperatorType operatorType)
              where TEntity : class, IDataObject, new()
        {
            foreach (var trigger in DependencyHelper.GetServices<IDataTrigger>())
        ...
        /// <returns></returns>
        public static void ConditionSqlCode<TEntity>(DataAccessOption<TEntity> option, List<string> conditions)
            where TEntity : class, IDataObject, new()
        {
            foreach (var trigger in DependencyHelper.GetServices<IDataTrigger>())
        ...
        /// <returns></returns>
        public static void BeforeUpdateSql<TEntity>(DataAccessOption<TEntity> option, StringBuilder code, string condition)
             where TEntity : class, IDataObject, new()
        {
        ...
        /// <param name="code">写入SQL的文本构造器</param>
        /// <returns></returns>
        public static void AfterUpdateSql<TEntity>(DataAccessOption<TEntity> option, StringBuilder code, string condition)
             where TEntity : class, IDataObject, new()
        {
            foreach (var trigger in DependencyHelper.GetServices<IDataTrigger>().Where(p => p.DataBaseType.HasFlag(option.DataBaseType)))
            {
