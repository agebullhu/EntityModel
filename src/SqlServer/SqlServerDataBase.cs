...
namespace Agebull.EntityModel.SqlServer
{
    ...
    ///     表示SQL SERVER数据库对象
    /// </summary>
    public class SqlServerDataBase : SimpleConfig, IDataBase
    {
        #region 事务
        ...
        /// </summary>
        /// <returns></returns>
        public IDisposable CreateDataBaseScope() => SqlServerDataBaseScope.CreateScope(this);
        ...
        {
            return ConfigurationManager.ConnectionStrings["SqlServer"];
        }
        ...
        public int Execute(string sql, params object[] args)
        {
            //using (SqlServerDataBaseScope.CreateScope(this))
            {
                return args.Length == 0
                    ? ExecuteInner(sql)
                    : ExecuteInner(sql, args.Select(p => p is SqlParameter parameter ? parameter : new SqlParameter { Value = p }).ToArray());
            }
        }
