...
namespace System
{
    ...
    public static class TypeExtend
    {
        ...
        {
            ConstructorInfo typeConstructor = resInfo.GetConstructor(Type.EmptyTypes);
            ...
            //构造匿名方法
            DynamicMethod callMethod = new DynamicMethod(methodName, typeof(TRes), new[] { typeof(TArg) });
            //构造动态IL（方法内部实现）
        ...
        public static Func<TRes> CreateFunc<TRes>(TypeInfo callInfo, string methodName, TypeInfo resInfo)
        {
            ...
            //构造匿名方法
            DynamicMethod callMethod = new DynamicMethod(methodName, typeof(TRes), null);
            //构造动态IL（方法内部实现）
            var il = callMethod.GetILGenerator();
            il.Emit(OpCodes.Nop);
            //1 调用对象构造
