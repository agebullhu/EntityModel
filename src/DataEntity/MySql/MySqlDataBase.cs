        ...
        public int Execute(string sql, params object[] args)
        {
            //using (MySqlDataBaseScope.CreateScope(this))
            {
                return args.Length == 0
                    ? ExecuteInner(sql)
                    : ExecuteInner(sql, args.Select(p => p is MySqlParameter ? (MySqlParameter)p : new MySqlParameter { Value = p }).ToArray());
            }
        }
        ...
        public object ExecuteScalar(string sql, params object[] args)
        {
            //using (MySqlDataBaseScope.CreateScope(this))
            {
                return args.Length == 0
                    ...
                    : ExecuteScalarInner(sql, args.Select(p => p is MySqlParameter ? (MySqlParameter)p : new MySqlParameter { Value = p }).ToArray());
            }
        }
