...
namespace Gboxt.Common.DataModel.SqlServer
{
    ...
    public class SqlServerDataBase : IDisposable
    {
        ...
        {
            LogRecorder.RecordDataLog("/***************************************************************/");
            var sqlParameters = args as SqlParameter[] ?? args.ToArray();
            foreach (var par in sqlParameters.Where(p => p != null))
            {
                LogRecorder.RecordDataLog(string.Format("declare @{0} {1};", par.ParameterName, par.SqlDbType));
            }
            foreach (var par in sqlParameters.Where(p => p != null))
            {
                LogRecorder.RecordDataLog(string.Format("SET @{0} = '{1}';", par.ParameterName, par.Value));
            }
            LogRecorder.RecordDataLog(sql);
            LogRecorder.RecordDataLog("GO");
        }
        ...
        public TData GetData<TData>(int table, int id) where TData : EditDataObject
        {
            ...
            if (!this._dataCache.TryGetValue(table, out tableDatas))
            {
                return null;
            }
            EditDataObject data;
        ...
        public TData TryAddToCache<TData>(int table, int id, TData data) where TData : EditDataObject
        {
            ...
            if (!this._dataCache.TryGetValue(table, out tableDatas))
            {
                this._dataCache.Add(table, tableDatas = new Dictionary<int, EditDataObject>());
            }
