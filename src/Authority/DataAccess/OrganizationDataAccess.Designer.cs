...
namespace Agebull.SystemAuthority.Organizations.DataAccess
{
    ...
    public partial class OrganizationDataAccess
    {
        ...
        {
            get { return Authorities.Table_Organization; }
        }
...
SELECT @@IDENTITY;";
        ...
        protected sealed override string UpdateSqlCode
        {
            ...
            {
                return @"
UPDATE `tb_sys_organization` SET
       `type` = ?Type,
       `code` = ?Code,
       `full_name` = ?FullName,
 ...
 WHERE `id` = ?Id;";
        ...
        /// 取得仅更新的SQL语句
        /// </summary>
        internal string GetModifiedSqlCode(OrganizationData data)
        {
            if (data.__EntityStatusNull || !data.__EntityStatus.IsModified)
                return ";";
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("UPDATE `tb_sys_organization` SET");
            //机构类型
            if (data.__EntityStatus.ModifiedProperties[OrganizationData.Real_Type] > 0)
                sql.AppendLine("       `type` = ?Type");
            //编码
            if (data.__EntityStatus.ModifiedProperties[OrganizationData.Real_Code] > 0)
                sql.AppendLine("       `code` = ?Code");
            //全称
            if (data.__EntityStatus.ModifiedProperties[OrganizationData.Real_FullName] > 0)
                sql.AppendLine("       `full_name` = ?FullName");
            //简称
            if (data.__EntityStatus.ModifiedProperties[OrganizationData.Real_ShortName] > 0)
                sql.AppendLine("       `short_name` = ?ShortName");
            //树形名称
            if (data.__EntityStatus.ModifiedProperties[OrganizationData.Real_TreeName] > 0)
                sql.AppendLine("       `tree_name` = ?TreeName");
            //级别
            if (data.__EntityStatus.ModifiedProperties[OrganizationData.Real_OrgLevel] > 0)
                sql.AppendLine("       `org_level` = ?OrgLevel");
            //机构标识
            if (data.__EntityStatus.ModifiedProperties[OrganizationData.Real_OrgId] > 0)
                sql.AppendLine("       `org_id` = ?OrgId");
            //备注
            if (data.__EntityStatus.ModifiedProperties[OrganizationData.Real_Memo] > 0)
                sql.AppendLine("       `memo` = ?Memo");
            //数据状态
            if (data.__EntityStatus.ModifiedProperties[OrganizationData.Real_DataState] > 0)
                sql.AppendLine("       `data_state` = ?DataState");
            //数据是否已冻结
            if (data.__EntityStatus.ModifiedProperties[OrganizationData.Real_IsFreeze] > 0)
                sql.AppendLine("       `is_freeze` = ?IsFreeze");
            //制作人
            if (data.__EntityStatus.ModifiedProperties[OrganizationData.Real_AuthorID] > 0)
                sql.AppendLine("       `author_id` = ?AuthorID");
            //制作时间
            if (data.__EntityStatus.ModifiedProperties[OrganizationData.Real_AddDate] > 0)
                sql.AppendLine("       `add_date` = ?AddDate");
            //最后修改者
            if (data.__EntityStatus.ModifiedProperties[OrganizationData.Real_LastReviserID] > 0)
                sql.AppendLine("       `last_reviser_id` = ?LastReviserID");
            //最后修改日期
            if (data.__EntityStatus.ModifiedProperties[OrganizationData.Real_LastModifyDate] > 0)
                sql.AppendLine("       `last_modify_date` = ?LastModifyDate");
            //审核状态
            if (data.__EntityStatus.ModifiedProperties[OrganizationData.Real_AuditState] > 0)
                sql.AppendLine("       `audit_state` = ?AuditState");
            //审核人
            if (data.__EntityStatus.ModifiedProperties[OrganizationData.Real_AuditorId] > 0)
                sql.AppendLine("       `auditor_id` = ?AuditorId");
            //审核时间
            if (data.__EntityStatus.ModifiedProperties[OrganizationData.Real_AuditDate] > 0)
        ...
        ///  所有字段
        /// </summary>
        static string[] _fields = new string[]{ "Id","Type","Code","FullName","ShortName","TreeName","OrgLevel","ParentId","OrgId","Memo","DataState","IsFreeze","AuthorID","AddDate","LastReviserID","LastModifyDate","AuditState","AuditorId","AuditDate" };
        ...
        /// 生成数据库访问范围
        /// </summary>
        internal static MySqlDataTableScope<OrganizationData> CreateScope()
        {
            var db = Authorities.Default ?? new Authorities();
            return MySqlDataTableScope<OrganizationData>.CreateScope(db, db.Organizations);
        }
        #endregion
    ...
    partial class Authorities
    {
        ...
        /// <summary>
        /// 机构(tb_sys_organization):机构
        /// </summary>
        public const int Table_Organization = 0x50001;


        /// <summary>
        /// 机构的结构语句
        /// </summary>
        private TableSql _tb_sys_organizationSql = new TableSql
        {
