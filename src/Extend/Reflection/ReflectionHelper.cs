...
namespace Agebull.Common.Reflection
{
    ...
    public sealed class ReflectionHelper
    {
        ...
        {
            var fields = IOHelper.GetAllField(path, "cs");
        ...
        public static object CreateObject(string typeName)
        {
            ...
            foreach (var asa in AppDomain.CurrentDomain.GetAssemblies())
            {
                try
                {
                    var type = asa.GetType(typeName);
                    if (type == null)
                    {
                        continue;
                    }
        ...
        public static object CreateGenericObject(Assembly assembly, Type generic, params Type[] genericArguments)
        {
            ...
            sb.Append("]]");
            var type = assembly.GetType(sb.ToString());
            return type == null
                    ? null
                    : CreateObject(type);
        }
        ...
        public static object CreateGenericObject(Type generic, params Type[] genericArguments)
        {
            ...
            {
                var re = CreateGenericObject(assembly, generic, genericArguments);
                if (re != null)
                {
                    return re;
                }
        ...
        private static void GetKnowTypes(ICollection<Type> knows, IEnumerable<Type> types, Type type)
        {
            ...
            foreach (var tp in enumerable.Where(p => !knows.Contains(p) && p.IsSubclassOf(type)))
            {
                if (!tp.IsGenericType)
                {
                    knows.Add(tp);
        ...
        private static string ToResult(Type type, object value)
        {
            ...
            var xml = doc.LastChild.OuterXml;
            return xml;
        }
        ...
        private static string Execuest(object obj, Dictionary<string, string> args, MethodInfo mi)
        {
            ...
            var pars = new object[infos.Length];
            foreach (var info in infos)
            {
                try
                {
                    var o = BuilderArg(info.ParameterType, args[info.Name]);
...
==================================={0}====================================
       ...
       public void {1} CopyFrom({0} obj)
        {{",
            ...
            sb.AppendLine("             {");
            foreach (var pi in type.GetProperties(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.DeclaredOnly | BindingFlags.Instance))
            {
                if (pi.CanWrite)
                {
        ...
        public static string ReadExctption(Exception err, string ti)
        {
            ...
            if (type.IndexOf("FaultException", StringComparison.Ordinal) >= 0)
            {
                ...
                if (innererr != null)
                {
                    var reason = TryGetProperty(tp, "Reason", err);
                    var errorCode = TryGetProperty(tp, "ErrorCode", err);
                    var errorMessage = TryGetProperty(tp, "ErrorMessage", err);
                    sb.AppendFormat(
#if DEBUG
"{4}Fault Type:{0}<br/>" +
#endif
...
"{0}Exception:{1}<br/>" +
 ...
 "{0}Source:{3}<br/>" + "{0}StackTrace:<br/>{0}   {4}<br/>" +
        ...
        {
            var xe = new XElement("Exceptions", new XElement("RootMessage", rootMessage));
        ...
        public static void SerializeException(Exception err, XElement par)
        {
            ...
            var inner = new XElement("InnerException");
            xe.Add(inner);
