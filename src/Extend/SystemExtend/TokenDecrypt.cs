...
namespace Agebull.Common.Security
{
    ...
    public class XmlDecrypt : XmlHelper
    {
        ...
        public static string Decrypt(string xml)
        {
            ...
            xr.Read() ;
            var cername = xr.ReadContentAsString().Trim() ;
            xr.ReadToNextSibling("X509SerialNumber") ;
            xr.Read() ;
            var cersn = xr.ReadContentAsString().Trim() ;
            xr.Close() ;
        ...
        {
            var x = CertificateHelper.GetCertificate(StoreName.My , StoreLocation.LocalMachine , cername) ;
            var thisSN = Asn1IntegerConverter.Asn1IntegerToDecimalString(x.GetSerialNumber()) ;
            if(thisSN != cersn)
            {
